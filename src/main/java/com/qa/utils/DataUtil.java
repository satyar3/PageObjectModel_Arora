...
public class DataUtil
{
	...
	public static boolean isSuiteRunnable(String suiteName)
	{
		...
		ExcelReader excel = new ExcelReader(Constants.TESTSUITE_XL_PATH);
	...
	public static Object[][] getData(String testCase, ExcelReader excel)
	{
		...
		System.out.println("Test case "+testCase+" starts from row num: " + testCaseRowNum);
		...
		System.out.println("Total rows of TestData for TestCase "+testCase + " are : " + testRows);
		...
		while (!excel.getCellData(Constants.DATA_SHEET, testDataCols, colStartRowNum).equals(""))
		{
		...
		System.out.println("Total cols for TestCase "+testCase+" are : " + testDataCols);
		...
		for (int rNum = dataStartRowNum; rNum < (dataStartRowNum + testRows); rNum++)
		{
			...
			for (int cNum = 0; cNum < testDataCols; cNum++)
			{
				...
				String colName = excel.getCellData(Constants.DATA_SHEET, cNum, colStartRowNum);
